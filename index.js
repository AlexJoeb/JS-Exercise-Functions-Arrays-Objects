...
function getLastCarInfo(inv) {
  const { car_make, car_model } = inv[inv.length - 1];
  return `This is a ${car_make} ${car_model}`;
}
...
function sortCarInventory(inv) {
  return inv.sort((a, b) => a.car_model.split("")[0] < b.car_model.split("")[0] ? -1 : 1);
}
...
function getGermanCars(inv) {
  ...
  for(let i in inv){
    const car = inv[i];
    if(models.includes(car.car_make)) cars.push(car);
    else continue;
  }
  return cars;
}
